...
class Exercise145a(Scene):
    def construct(self):
        ...
        label1 = Text("X", color=BLACK, font_size=50).move_to(circle_1).shift(UP*3)

        label2 = Text("Y", color=BLACK, font_size=50).move_to(circle_2).shift(UP*3)

        label3 = MathTex("1", color=BLACK).scale(1.9).move_to(ORIGIN).shift(UP*0.8)
        label31 = MathTex("3", color=BLACK).scale(1.9).next_to(label3, DOWN, buff=1.1)
        label4 = MathTex("5", color=BLACK).scale(2).move_to(ORIGIN).shift(LEFT*2)
        label41 = MathTex("2", color=BLACK).scale(2).move_to(ORIGIN).shift(RIGHT*2)

...
class Exercise8112c(Scene):
    def construct(self):
        ...
        def mid_tip_towards(start_point, end_point, length=0.45):
            ...
            seg = Line(mid - direction * length / 2,
        ...
        tip_BC = mid_tip_towards(C, B)  
        tip_CD = mid_tip_towards(C, D)   
        tip_FC = mid_tip_towards(C, F)   
        ...
        def double_tick(line, length=0.3, gap=0.08):
            ...
            tick1 = Line(mid - normal * length / 2 + direction * gap,
                         mid + normal * length / 2 + direction * gap,
                         color=BLACK,
                         stroke_width=3)

            tick2 = Line(mid - normal * length / 2 - direction * gap,
                         mid + normal * length / 2 - direction * gap,
                         color=BLACK,
                         stroke_width=3)

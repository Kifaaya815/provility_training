...
class Exercise813c(Scene):
    def construct(self):
        ...
        dot_P = Dot(P, color=PURE_RED).next_to(P, LEFT * 4)
        dot_Q = Dot(P, color=PURE_RED).next_to(P, RIGHT * 4)

        label_2 = MathTex("2", color=PURE_BLUE).next_to(dot_P, DOWN*1.5+RIGHT*0.2)
        label_1 = MathTex("1", color=PURE_BLUE).next_to(dot_Q, DOWN*1.6+RIGHT*1.7)
        ...
        OP_line = Line(O_shifted, dot_Q.get_center(), color=PURE_BLUE, stroke_width=3)

        OP_mid = OP_line.point_from_proportion(0.5)
        OP_tip = StealthTip(length=0.28, color=BLACK)
        OP_tip.rotate(OP_line.get_angle())
        OP_tip.move_to(OP_mid).shift(RIGHT*0.09+UP*0.5)

...
class Exercise8512(Scene):
    def construct(self):
        ...
        label_mn = MathTex("m : n", color=PURE_RED).next_to(P, UP*3.5)

        # ---------------- RED CURVE (m:n → a vector) ----------------
        curve_start = P + UP*1.4+RIGHT*0.4            # lift start higher
        curve_control1 = curve_start + UP*1.8 # vertical tangent → circular feel
        curve_control2 = np.array([-7, 3, 0])  # left of AO
        curve_end = np.array([-2.7, -0.2, 0])      # near a-vector label

        red_curve = CubicBezier(
            curve_start,
            curve_control1,
            curve_control2,
            curve_end,
        ...
        # Tip direction using curve tangent at the end
        ...
        # Tip direction for second curve

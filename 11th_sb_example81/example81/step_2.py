...
class Example81_1(Scene):
    def construct(self):
        ...
        Q= Line(start=axes.c2p(0, 0),
                ...
                length*np.cos(angle),
                length*np.sin(angle)), color=BLUE_D, stroke_width=8).set_z_index(-1)
        ...
        distance_label = MathTex(r"60\,\text{km}", color=BLACK, font_size=40)
        ...
        O_label = Text("O", color=BLACK, font_size=36).move_to(axes.c2p(-0.29, 0.29))
